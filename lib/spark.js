...
function Spark(opts) {
  ...
  this.connect(function(error, data) {
    ...
    if (error !== null) {
      ...
      this.emit("error", new Error("Unable to connect to the voodoospark firmware, has it been loaded?"));
    } else {
...
Spark.prototype.connect = function(handler) {
  ...
  request = https.get(url + action+"?access_token="+state.token, function (res) {
    if (res.statusCode === 200) {
      ...
      if (handler) {
        handler(new Error("Unable to connect to spark cloud."));
      } else {
        throw new Error("Unable to connect to spark cloud.");
      }
    }
  });
